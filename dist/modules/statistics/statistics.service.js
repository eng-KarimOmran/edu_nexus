...
const DashboardService = {
    clients: async ({ dataSafe, tx }) => {
        ...
        const baseWhere = { academyId, createdAt: dateRange };
        const run = async (tx) => {
    ...
    subscriptions: async ({ dataSafe, tx }) => {
        ...
        const run = async (tx) => {
            ...
            const SUBSCRIPTION_STATUSES = Object.values(enums_1.SubscriptionStatus);
            const result = Object.fromEntries(SUBSCRIPTION_STATUSES.map(status => [
                status,
    ...
    ledgerTransaction: async ({ dataSafe, tx }) => {
        ...
        const run = async (tx) => {
            ...
            const find = (type, method) => Number(stats.find(s => s.transactionType === type &&
                (method === undefined || s.paymentMethod === method))?._sum.amount ?? 0);
    ...
    lessons: async ({ dataSafe, tx }) => {
        ...
        const { startDate, endDate } = dataSafe.query;
        const dateRange = { gte: startDate, lte: endDate };
        const baseWhere = { academyId, endTime: dateRange };
        const run = async (tx) => {
            ...
            const getStatus = (s) => statusStats.find(x => x.lessonStatus === s)?._count.id ?? 0;
            const getTransmission = (t) => transmissionStats.find(x => x.transmission === t)?._count.id ?? 0;
    ...
    area: async ({ dataSafe, tx }) => {
        ...
        const run = async (tx) => {
            ...
            if (areaStats.length === 0)
                return [];
            const areaIds = areaStats.map(a => a.areaId).filter(Boolean);
    ...
    car: async ({ dataSafe, tx }) => {
        ...
        const run = async (tx) => {
            ...
            if (carStats.length === 0)
                return [];
            const carIds = carStats.map(c => c.carId).filter(Boolean);
    ...
    captain: async ({ dataSafe, tx }) => {
        ...
        const { startDate, endDate } = dataSafe.query;
        const dateRange = { gte: startDate, lte: endDate };
        const baseWhere = { academyId, endTime: dateRange, lessonStatus: "COMPLETED" };
        const run = async (tx) => {
            ...
            const ids = captainStats.map(c => c.jobProfileId).filter(Boolean);
            if (ids.length === 0)
                return [];
    ...
    usersCreatedSubscription: async ({ dataSafe, tx }) => {
        ...
        const run = async (tx) => {
            ...
            const ids = jobProfileStats.map(j => j.createdById).filter(Boolean);
            if (ids.length === 0)
                return [];
